...
class Activity extends CRMEntity {
	...
	var $tab_name = Array('vtiger_crmentity',
			      'vtiger_activity',
			      'vtiger_activitycf',
			    'vtiger_vehiculeactivityrel',
			//'vtiger_seactivityrel',
			//'vtiger_cntactivityrel',
			      );
	
	//SG1409 In our case, we use vtiger_vehiculeactivityrel to have a related Vehicule field
	var $tab_name_index = Array('vtiger_crmentity'=>'crmid',
				   ...
				   'vtiger_seactivityrel'=>'activityid',
				    ...
				    'vtiger_vehiculeactivityrel'=>'activityid');
	...
	var $additional_column_fields = Array('assigned_user_name', 'assigned_user_id', 'contactname', 'contact_phone', 'contact_email', 'parent_name','vehiculeid');
	...
	function save_module($module)
	{
		...
		if(isset($_REQUEST['vehiculeidlist']) && $_REQUEST['vehiculeidlist'] != '') {
			...
			$vehiculeIdsList = explode (';', $_REQUEST['vehiculeidlist']);
			$count = count($vehiculeIdsList);

			$sql = 'INSERT INTO vtiger_vehiculeactivityrel (vehiculeid, activityid) VALUES ';
			for($i=0; $i<$count; $i++) {
				$sql .= " (". $vehiculeIdsList[$i].", ".$recordId.")";
				if ($i != $count - 1) {
					$sql .= ',';
				}
			}
...
//calendarsync
	...
	//SG1409?? Useless ??
			"Vehicules" => array("vtiger_vehiculeactivityrel"=>array("activityid","vehiculeid"),"vtiger_activity"=>"activityid")
		);
