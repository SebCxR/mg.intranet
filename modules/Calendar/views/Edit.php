...
Class Calendar_Edit_View extends Vtiger_Edit_View {
	...
	foreach($requestFieldList as $fieldName=>$fieldValue){
		...
		/*
		//TODO default fields for mg
		if (empty($record) && ($fieldName == 'assigned_user_id')) {
			$fieldValue = '7';			
		}
		*/
		...
		//SG1409
		$existingRelatedVehicules = $recordModel->getRelatedVehiculeInfo();
		...
		if($isRelationOperation) {		
			switch  ($request->get('sourceModule')) {			
				case 'Contacts' :
						$fromContactIdValue = $request->get('sourceRecord');
					if(!empty($fromContactIdValue)) {
