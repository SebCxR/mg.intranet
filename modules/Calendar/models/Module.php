...
class Calendar_Module_Model extends Vtiger_Module_Model {
	...
	public function getSideBarLinks($linkParams) {
		...
		$quickLinks = array(
			...
			array(
				...
				'linkurl' => $this->getVehiculesCalendarViewUrl(),
				'linkicon' => '',
			),
			array(
				'linktype' => 'SIDEBARLINK',
		...
		if ($linkParams['ACTION'] == 'InvitedCalendar') {
			$quickWidgets[] = array(
				'linktype' => 'SIDEBARWIDGET',
		...
		if ($linkParams['ACTION'] == 'VehiculesCalendar') {
			$quickWidgets[] = array(
				'linktype' => 'SIDEBARWIDGET',
				'linklabel' => 'LBL_VEHICULES_CALENDARS',
				'linkurl' => 'module='.$this->get('name').'&view=ViewTypes&mode=getVehiculesListForCalendar',
				'linkicon' => ''
			);
		}
	...
	*/
	...
	public static function getVehiculesForCalendar($criteria) {			
		$db = PearDatabase::getInstance();
		$query = "SELECT vtiger_vehicules.vehiculesid AS vehiculeid, vtiger_vehicules.vehicule_name, vtiger_vehicules.calcolor
		...
		$vehiculeIds = Array();
		for($i=0; $i<$rows; $i++){
			$id = $db->query_result($result,$i,'vehiculeid');
			$vehiculeIds[$id] = Array();
			
			$vehiculeName = $db->query_result($result,$i,'vehicule_name');
			$vehiculeColor = $db->query_result($result,$i,'calcolor');
			$vehiculeIds[$id]['name'] = $vehiculeName;
			$vehiculeIds[$id]['color'] = $vehiculeColor;
		}
		return $vehiculeIds;
	}
	/**
	...
	*/
	...
	public static function getInvitedUsersForCalendar($criteria) {			
		$db = PearDatabase::getInstance();
		$query = "SELECT vtiger_users.first_name,vtiger_users.last_name, vtiger_users.id as userid
			FROM vtiger_users
	...
	/**
	 *  Function returns the url for Vehicules Calendar view
	 * @return <String>
	 */
	 ...
	 * @return <String>
	 */
	public function getInvitedCalendarViewUrl() {
		return 'index.php?module=Calendar&view=InvitedCalendar';
	}
	/**
