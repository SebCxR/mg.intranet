...
{strip}
    ...
    <div class="popupContainer row-fluid">
        <div class="span12">            
                    <span class="logo span2"><img src="{$COMPANY_LOGO->get('imagepath')}" title="{$COMPANY_LOGO->get('title')}" alt="{$COMPANY_LOGO->get('alt')}"/></span>
		    {if $SOURCE_MODULE and ($SOURCE_MODULE eq 'MGTransports')}
                    <span class="recordLabel" title="{$SOURCE_RECORD_INSTANCE->getName()}">				
                                 <span class="titleinfo"><b>{vtranslate('LBL_BUILDING_VEHICULES_LIST', $MODULE_NAME)}</b></span>
                                <span class="subject"> <b>{vtranslate($SOURCE_RECORD_INSTANCE->getDisplayValue('subject'), $PARENT_MODULE)} - </b></span>
                                <span class="mgtypetransport"><b>{vtranslate($SOURCE_RECORD_INSTANCE->getDisplayValue('mgtypetransport'), $PARENT_MODULE)} - </b></span>
                                 <span class="datetransport"><b>{vtranslate($SOURCE_RECORD_INSTANCE->getDisplayValue('datetransport'), $PARENT_MODULE)}</b></span>
    ...
    <form class="form-horizontal popupSearchContainer">
        <div class="control-group margin0px">
            ...
            <input type="text" placeholder="{vtranslate('LBL_TYPE_SEARCH')}" id="searchvalue" {if $SEARCH_KEY and $SEARCH_VALUE} value="{$SEARCH_VALUE}" {/if}/>
                ...
                <select style="width: 150px;" class="chzn-select help-inline" id="searchableColumnsList">
                    {foreach key=block item=fields from=$RECORD_STRUCTURE}
                        {foreach key=fieldName item=fieldObject from=$fields}
                            <optgroup>
                                <option value="{$fieldName}" {if $SEARCH_KEY and ($SEARCH_KEY eq $fieldObject->get('name'))} selected {/if}> {vtranslate($fieldObject->get('label'),$MODULE)} </option>
                            </optgroup>
                        {/foreach}
                    {/foreach}
                </select>
        ...
        <div class="popupPaging">
            <div class="row-fluid">
                <span class="actions span6">&nbsp;
                    {if $MULTI_SELECT}
                    <button class="select btn"><strong>{vtranslate('LBL_SELECT', $MODULE)}</strong></button>
                {/if}
            </span>
            <span class="span6">
