...
{strip}
	...
	<table class="table table-bordered equalSplit detailview-table">
			...
			{else}
				...
				{else}
				 ...
				 <td class="fieldValue {$WIDTHTYPE}" id="{$MODULE}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
					 ...
					 {* ED141016 ça fout bien la merdouille dans l'enregistrement quand on valide un champ
						un event est devenu une tâche, je crois *}
					 {if FALSE && $IS_AJAX_ENABLED && $FIELD_MODEL->isEditable() eq 'true' && ($FIELD_MODEL->getFieldDataType()!=Vtiger_Field_Model::REFERENCE_TYPE) && $FIELD_MODEL->isAjaxEditable() eq 'true'}
		...
		{* ED141016 
		"Choisir les chauffeurs" n'est pas un champ de la table vtiger_field
		*}
		{if $BLOCK_LABEL_KEY eq "LBL_RELATED_VEHICULE_INFORMATION"}		
			<tr>
				<td class="fieldLabel {$WIDTHTYPE}"><label class="muted pull-right marginRight10px">{vtranslate('LBL_INVITEMG_USERS',$MODULE_NAME)}</label></td>
				<td class="fieldValue {$WIDTHTYPE}">
					...
					{/foreach}
					 <span class="hide edit">
						...
						{foreach key=USER_ID item=USER_NAME from=$ACCESSIBLE_USERS}
							{if $USER_ID eq $USER_MODEL->getId()}
								{continue}
							{/if}
						...
						{/foreach}
						<select><i>TODO : 
						1) &ccedil;a ne valide pas par ajax<br>
						2) l'affichage par l'onglet Detail ne met pas le select en picklist (manque un js avec registerEvent pour initialiser $(..).select2</i>
					</span>
				</td>
			</tr>
		{/if}
