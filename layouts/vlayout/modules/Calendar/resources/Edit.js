...
Vtiger_Edit_Js("Calendar_Edit_Js",{
...
},{//All SG1409 tagged insertion about the Vehicule related module
    relatedVehiculeElement : false,
    ...
    //SG1409
    getRelatedVehiculeElement : function() {
        if(this.relatedVehiculeElement == false) {
            this.relatedVehiculeElement =  jQuery('#vehiculeid_display');
        }
        return this.relatedVehiculeElement;
    },
    //SG1409 End
    isEvents : function() {
    ...
    getPopUpParams : function(container) {
        ...
        //SG1409 add 'vehiculeid' in condition. Seems unnecessary for the moment
        if(!this.isEvents() || ((sourceFieldElement.attr('name') != 'contact_id') && (sourceFieldElement.attr('name') != 'vehiculeid'))) {
            return params;
        }
        var form = this.getForm();
    ...
    //SG1409
    fillRelatedVehicules : function() {
        var form = this.getForm();
        var relatedVehiculeValue = form.find('[name="relatedVehiculeInfo"]').data('value');
        for(var vehiculeId in relatedVehiculeValue) {
            var info = relatedVehiculeValue[vehiculeId];
            info.text = info.name;
            relatedVehiculeValue[vehiculeId] = info;
        }
        this.getRelatedVehiculeElement().select2('data',relatedVehiculeValue);
    },
    ...
    //SG1409
    addNewVehiculeToRelatedList : function(newVehiculeInfo){
         var resultentData = new Array();
            var element =  jQuery('#vehiculeid_display');
            var selectContainer = jQuery(element.data('select2').containerSelector);
            var choices = selectContainer.find('.select2-search-choice');
            choices.each(function(index,element){
                resultentData.push(jQuery(element).data('select2-data'));
            ...
            var select2FormatedResult = newVehiculeInfo.data;
            for(var i=0 ; i < select2FormatedResult.length; i++) {
              var recordResult = select2FormatedResult[i];
              recordResult.text = recordResult.name;
              resultentData.push( recordResult );
            }
            jQuery('#vehiculeid_display').select2('data',resultentData);
    },
...
//SG add vehicule treatment Function Renamed (original name = registerRelatedContactSpecificEvents )
    registerRelatedContactAndVehiculeSpecificEvents : function() {
        var thisInstance = this;
	...
	this.getRelatedVehiculeElement().select2({
             minimumInputLength: 3,
             ajax : {
                'url' : 'index.php?module=Vehicules&action=BasicAjax&search_module=Vehicules',
                'dataType' : 'json',
                'data' : function(term,page){
                     var data = {};
                     data['search_value'] = term;
	...
	registerEvents : function(){
        ...
        this.registerRelatedContactAndVehiculeSpecificEvents();
		this._super();
	}
});